...
P = np.loadtxt('Py_s.txt')
# Times for mean
N = len(y)
path_ts = np.linspace(-11.6, 0., N)
vs = P[range(N), range(N)]

np.savetxt('y_south.txt', y[:-1])
np.savetxt('Py_south.txt', P[:-1, :-1])
...
# Random samples
samples = np.random.multivariate_normal(y, P, 25)

thing = []
for i in range(len(samples)):
    plt.plot(path_ts, samples[i], lw = 1.5)

    L_indexes = np.where(samples[i] > Ls[3])
