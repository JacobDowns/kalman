...
plt.title('(a)')
ts = np.linspace(obs_ages.min(), -7.3e3, 300)
#[11, 2, 5, 8]
...
plt.plot(ts, dt_functions[5](ts), color = current_palette[-1], lw = 2.1, label = 'Jun.')
...
ages = np.loadtxt('default_runs/center1/age.txt')
Ls1 = np.loadtxt('default_runs/center1/L.txt') / 1000.
Ls2 = np.loadtxt('default_runs/south1/L.txt') / 1000.
...
for i in [0, 1, 2, 3, 4]:
    plt.plot([ages.min(), -7250.], [L1_obs[i], L1_obs[i]], 'gray', linestyle=':', alpha = 1., lw = 2)

plt.plot(ages, Ls1, 'k', lw = 3.5)
#plt.grid(color='lightgray', linestyle=':', linewidth=2.5)
plt.xticks([-11e3, -10e3, -9e3, -8e3, -7e3])
plt.xlim([obs_ages.min(), -7250.])
plt.ylim([245., 420.])
...
for i in [0, 1, 2, 3, 4]:
    plt.plot([ages.min(), -7250.], [L2_obs[i], L2_obs[i]], 'gray', linestyle=':', alpha = 1., lw = 2)
