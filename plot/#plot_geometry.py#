...
################################################################################
dt = 1./3.
model_inputs = PaleoInputs('paleo_inputs/north_steady_seasonal2.h5', {})
...
obs_Ls = [449441.8117461069, 409256.6118172061, 334915.6887022482, 302077.4579612268, 289091.57418598986, 289091.57418598986]
...
plt.plot(xs, surface.compute_vertex_values(), color = current_palette[0], linewidth = 3.5, alpha = 0.8)
plt.plot(xs, bed.compute_vertex_values(), 'k', linewidth = 3.5)

plt.ylim([-300., 4500.])
plt.xlim([0., 450e3 + 2.5e3])
...
print (surface.compute_vertex_values() - bed.compute_vertex_values()).max()
quit()
