...
plt.plot(ts, delta_temp_djf(ts), color = current_palette[0], lw = 3.5, label = 'DJF', alpha = 0.85)
plt.plot(ts, delta_temp_mam(ts), color = current_palette[-3], lw = 3.5, label = 'MAM', alpha = 0.85)
plt.plot(ts, delta_temp_jja(ts), color = current_palette[-1], lw = 3.5, label = 'JJA', alpha = 0.85)
plt.plot(ts, delta_temp_son(ts), color = current_palette[2], lw = 3.5, label = 'SON', alpha = 0.85)
...
ticks = ax.get_xticks()
ax.set_xticklabels([int(abs(tick / 1000.)) for tick in ticks])
plt.fill_between(ages_fine, Ls2_interp(ages_fine) - 2.0*(np.sqrt(errors1)), Ls2_interp(ages_fine) + 2.0*(np.sqrt(errors1)), facecolor='gray', alpha = 0.33)
...
ticks = ax.get_xticks()
ax.set_xticklabels([int(abs(tick / 1000.)) for tick in ticks])
plt.fill_between(ages_fine, Ls3_interp(ages_fine) - 2.0*(np.sqrt(errors1)), Ls3_interp(ages_fine) + 2.0*(np.sqrt(errors1)), facecolor='gray', alpha = 0.33)
...
plt.tight_layout()
plt.savefig('seasonal_default.png', dpi=700)
